<script lang="ts">
	import type { ServicePricing, PricingTier, ActiveHoursConfig } from "$lib/types";
	...
	let config = $state<ServicePricing>({ tiers: [], fallbackDayPrice: 0, fallbackNightPrice: 0 });
	...
	async function load() {
		loading = true;
		try {
			const [pricing, hours] = await Promise.all([getPricing(serviceId), getActiveHours(serviceId)]);
			config = pricing;
			activeHours = hours;
			error = "";
		} catch (e) {
	...
	async function save() {
		...
		error = "";
		try {
			await updatePricing(serviceId, config);
	...
	async function saveHours() {
		...
		error = "";
		try {
			await updateActiveHours(serviceId, activeHours);
	...
	function addTier() {
		const lastTier = config.tiers.length > 0 ? config.tiers[config.tiers.length - 1] : undefined;
	...
	function removeTier(index: number) {
		config.tiers = config.tiers.filter((_, i) => i !== index);
	}

	function updateTier(index: number, field: keyof PricingTier, value: number) {
		config.tiers[index][field] = value;
	}
</script>
...
<div class="space-y-8 max-w-4xl mx-auto">
	<div>
		...
		<p class="text-muted-foreground">Preisgestaltung nach Entfernung und Tageszeit ({serviceLabel})</p>
	</div>
	...
	{:else}
		...
		<!-- Single pricing card for current service -->
		<Card.Root>
			<Card.Header>
				<div class="flex items-center gap-3">
					<div class="p-2 rounded-lg bg-slate-100">
						<EuroIcon class="h-5 w-5 text-slate-600" />
					</div>
					<div>
						<Card.Title>{serviceLabel}</Card.Title>
			...
			<Card.Content>
				<div class="space-y-6">
					...
					<table class="w-full">
						...
						<tbody>
							{#each config.tiers as tier, index}
								<tr>
									<td class="py-2 pr-3">
										<div class="relative">
											<Input
												...
												min="1"
												value={tier.minutes}
												onchange={(e) => updateTier(index, "minutes", parseInt((e.target as HTMLInputElement).value) || 0)}
												class="pr-12"
											/>
									...
									<td class="py-2 pr-3">
										<div class="relative">
											<Input
												...
												step="10"
												value={tier.dayPrice}
												onchange={(e) => updateTier(index, "dayPrice", parseInt((e.target as HTMLInputElement).value) || 0)}
												class="pr-8"
											/>
									...
									<td class="py-2 pr-3">
										<div class="relative">
											<Input
												...
												onchange={(e) => updateTier(index, "nightPrice", parseInt((e.target as HTMLInputElement).value) || 0)}
												class="pr-8"
											/>
											<EuroIcon class="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
										</div>
									...
									<td class="py-2">
										<Button variant="ghost" size="icon" onclick={() => removeTier(index)} class="text-red-500 hover:text-red-700 hover:bg-red-50">
					...
					<!-- Add tier button -->
					<Button variant="outline" onclick={addTier} class="w-full">
							...
							<div class="space-y-2">
								...
								<div class="relative">
									<Input
										...
										min="0"
										step="10"
										bind:value={config.fallbackDayPrice}
										class="pr-8"
									/>
									<EuroIcon class="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
								</div>
							...
							<div class="space-y-2">
								...
								<div class="relative">
									<Input
										...
										min="0"
										step="10"
										bind:value={config.fallbackNightPrice}
										class="pr-8"
									/>
									<EuroIcon class="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
								</div>
